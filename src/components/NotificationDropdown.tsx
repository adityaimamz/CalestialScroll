...
export function NotificationDropdown() {
    ...
    // Dialog state for report replies
    ...
    const handleNotificationClick = async (notification: Notification) => {
        ...
        if (notification.type === "admin_chapter_report") {
            navigate("/admin/reports/chapters");
        } else if (notification.type === "admin_comment_report") {
            navigate("/admin/reports/comments");
        } else if (notification.type === "admin_report") {
            // Fallback for old notifications
            if (notification.entity_id === "chapter_report") {
                navigate("/admin/reports/chapters");
            } else if (notification.entity_id === "comment_report") {
                navigate("/admin/reports/comments");
            } else {
                navigate("/admin");
            }
        }
        else if (notification.type === "report_reply") {


            try {
                let reply = null;
                // Try chapter reports
                const { data: chapReport } = await supabase
                ...
                if (chapReport) {
                    reply = (chapReport as any).admin_reply;
                } else {
                    // Try comment reports
                    const { data: comReport } = await supabase
                        .from("comment_reports" as any)
                        .select("admin_reply")
                        .eq("id", notification.entity_id)
                        .maybeSingle();
                    if (comReport) reply = (comReport as any).admin_reply;
                }

                setReplyContent(reply || "No message content found.");
        ...
        else if ((notification.type === "reply" || notification.type === "like") && notification.entity_id) {
            try {
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <p className="text-sm leading-snug">
                                                ...
                                                <span className="text-muted-foreground">
                                                    ...
                                                    {notification.type === "admin_report" && "New Report Submitted"}
                                                    {notification.type === "admin_chapter_report" && "New Chapter Report"}
                                                    {notification.type === "admin_comment_report" && "New Comment Report"}
                                                    {notification.type === "report_reply" && "Admin replied to your report"}
                                                </span>
                                            </p>
            ...
            <Dialog open={isMessageDialogOpen} onOpenChange={setIsMessageDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Response to your report from the administration team.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="py-4 text-sm bg-muted/30 p-4 rounded-md whitespace-pre-wrap">
                        {replyContent}
                    </div>
                    <DialogFooter>
